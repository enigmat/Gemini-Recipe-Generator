...
const CookingClassCard: React.FC<CookingClassCardProps> = ({ cookingClass, onClick }) => {
  return (
    <div
      className="bg-white rounded-lg shadow-md overflow-hidden cursor-pointer transform hover:scale-105 transition-all duration-300 border border-slate-100"
    ...
    >
      ...
      <div className="p-5">
        ...
        <h3 className="text-xl font-bold text-slate-900 mt-1">{cookingClass.title}</h3>
        <p className="text-slate-600 mt-2 text-sm">{cookingClass.description}</p>
      </div>
    </div>
  );
};
